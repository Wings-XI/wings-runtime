@echo off
powershell -command "&{(get-host).ui.rawui.windowsize=@{width=100;height=44};}"
...
start "import" /SEPARATE import.bat %combined%
...
:append
echo %~1 import
echo select '%~1'; >> %combined%
echo start transaction; >> %combined%
type ".\wings\sql-wings\%~1" >> %combined%
