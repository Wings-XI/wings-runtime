...
"%nssm%\nssm" install %servicename% "%workingDirectory%\topaz_game_64_d.exe"
"%nssm%\nssm" set %servicename% AppParameters "--ip %ip% --port %port% --log ..\..\..\servicelogs\Debug-%regionname%-%port%.log"
...
"%nssm%\nssm" set %servicename% AppEnvironmentExtra TRACY_NO_INVARIANT_CHECK=1
